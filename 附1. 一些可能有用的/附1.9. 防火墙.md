...
实际（在Ubuntu16.04.3中安装的firewalld）测试，添加`--permanent`选项只是在未重启时使用`firewall-cmd --reload`重载防火墙不会生效，但重启（reboot）还是会生效，可以添加与不添加命令连续运行，即可实现生效且重启后也有效。（不知道测试是否正确，且暂时没在CentOS7上测试，可以参考，或者按照本教程下面的方式来切换区域）
