...
*filter
# 修改系统默认的规则, 添加-i eth+
-A INPUT -m state --state RELATED,ESTABLISHED -i eth+ -j ACCEPT
# 在-A INPUT -p icmp -j ACCEPT下面添加
# 表示禁止10.8.0.6以外其他客户端访问服务器本机, 不影响连接服务器同网段的其他主机(根据需要选择设置)
-A INPUT -i tun+ -s 10.8.0.6 -j ACCEPT
-A INPUT -i tun+ -s 10.8.0.0/24 -j DROP
...
firewall-cmd --permanent --direct --add-rule ipv4 filter FORWARD 0 -o tun+ -j ACCEPT
firewall-cmd --direct --add-rule ipv4 filter FORWARD 0 -i tun+ -s 10.8.0.0/24 -j ACCEPT
firewall-cmd --direct --add-rule ipv4 filter FORWARD 0 -i tun+ -s 10.8.0.0/24 -m iprange --dst-range 192.168.10.50-192.168.10.60 -j ACCEPT  # 或是指定允许访问的范围
firewall-cmd --permanent --direct --add-rule ipv4 filter FORWARD 0 -i tun+ -s 10.8.0.0/24 -j ACCEPT
```
