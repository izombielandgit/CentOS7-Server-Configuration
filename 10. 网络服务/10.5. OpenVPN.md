...
补充一个微信提醒的示例（脚本参考<https://blog.csdn.net/bwlab/article/details/50725335>和<https://my.oschina.net/u/3658138/blog/1586428>）（很多内容不懂，所以只测试了能正常发送）：
...
local PartyID=1  # 部门ID，定义了范围，组内成员都可接收到消息(没有具体测试)
...
## 在表usersz中加入admin字段, 值为0表示非管理员, 值为1表示管理员
admin=`${RUN_MY}"SELECT admin FROM users WHERE user_id='${common_name}'" -N`
if [ ${admin} = 1 ]; then bash /etc/openvpn/scripts/connect-wx.sh && exit 0
else exit 0
fi
...
## 如果需要微信提醒加入下面内容, 不需要的话, 直接exit 0结束即可
bash /etc/openvpn/scripts/status-wx.sh
