...
```
firewall-cmd --permanent --add-service openvpn  # 或添加自己设置的端口
...
>CREATE TABLE `openvpn`.`log` (
  ...
  `log_trusted_ip` varchar(32) DEFAULT NULL,
  `log_trusted_port` varchar(10) DEFAULT NULL,
  `log_remote_ip` varchar(32) DEFAULT NULL,
  `log_remote_port` varchar(10) DEFAULT NULL,
  `log_protocol` varchar(10) DEFAULT NULL,
...
# `log_id` log编号（自动增长）
# `user_id` 本次登录用户账号
# `log_trusted_ip` 本次客户端外网IP
# `log_trusted_port` 本次客户端端口
# `log_remote_ip` 本次客户端获取到的虚拟IP
# `log_remote_port` 本次连接服务端端口号
# `log_protocol` 本次连接服务端端口协议类型（upd/tcp）
...
# `last_login_from` 最后一次连接用户客户端外网IP
