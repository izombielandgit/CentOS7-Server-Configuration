...
编辑`/etc/openvpn/script/disconnect.sh`文件，创建客户端断开连接时运行脚本（通过udp协议连接，退出会有延时；客户端异常退出好像会记录不准，没有具体测试）：
...
|id|int|
|user_id|varchar(50)|
|user_online_serv1|tinyint|
|login_times_serv1|int|
|last_login_serv1|datetime|
|last_logout_serv1|datetime|
|last_duration_serv1|int|
|last_login_from_serv1|varchar(50)|
|last_total_serv1|bigint|
|total_duration_serv1|bigint|
|total_received_serv1|bigint|
|total_sent_serv1|bigint|
|grand_total_serv1|bigint|
|user_online_serv2|tinyint|
|login_times_serv2|int|
|last_login_serv2|datetime|
|last_logout_serv2|datetime|
|last_duration_serv2|int|
|last_login_from_serv2|varchar(50)|
|last_total_serv2|bigint|
|total_duration_serv2|bigint|
|total_received_serv2|bigint|
|total_sent_serv2|bigint|
|grand_total_serv2|bigint|
...
sqlcmd -S $HOST -U $USER -P $PASS -d $DB -Q "if exists(select 1 from traffic where traffic.user_id='$common_name') update traffic set user_online_$SERV_N=1,login_times_$SERV_N=login_times_$SERV_N+1,last_login_$SERV_N=getdate(),last_login_from_$SERV_N='$trusted_ip' where user_id='$common_name' else insert into traffic(user_id,user_online_$SERV_N,login_times_$SERV_N,last_login_$SERV_N,last_login_from_$SERV_N) values('$common_name',1,1,getdate(),'$trusted_ip');"
# 在表log中插入登陆数据
...
sqlcmd -S $HOST -U $USER -P $PASS -d $DB -Q "update traffic set user_online_$SERV_N=0,last_logout_$SERV_N=getdate(),last_duration_$SERV_N=datediff(second,last_login_$SERV_N,getdate()),last_total_$SERV_N=$bytes_received+$bytes_sent,total_duration_$SERV_N=total_duration_$SERV_N+datediff(second,last_login_$SERV_N,getdate()),total_received_$SERV_N=total_received_$SERV_N+$bytes_received,total_sent_$SERV_N=total_sent_$SERV_N+$bytes_sent,grand_total_$SERV_N=grand_total_$SERV_N+$bytes_received+$bytes_sent where user_id='$common_name'"
```

两条语句可以使用分号隔开写在一行，保存后同样运行：

`chmod +x /etc/openvpn/script/*.sh`

选择一种类型的脚本后，编辑`/etc/openvpn/server.conf`文件：
