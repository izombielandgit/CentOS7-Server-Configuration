...
GitHub上的easy-rsa最新版本为easy-rsa3，生成证书命令有一些变化，如下操作：
...
./easyrsa gen-req server nopass  # 创建server端证书和private key，nopass表示不加密private key，然后提示设置Common Name
./easyrsa sign-req server server  # 给server端证书签名，确认信息后输入yes，然后输入build-ca时设置的密码
