...
CentOS6可以通过“pam\_mysql”使用MySQL存储用户名密码，但CentOS7好像已经取消“pam\_mysql”，自行编译什么的也弄了，但是无法使用。故使用RADIUS进行认证。
...
# 检查是否有以下内容(经测试即便这样编译安装，还是不支持MD5密码)
...
数据库新建一个用户`INSERT INTO `users` (`id`, `user_id`, `enable`, `user_pass`, `user_name`) VALUES (NULL, 'test', '1', PASSWORD('test'), '测试');`，在客户端尝试登陆。
