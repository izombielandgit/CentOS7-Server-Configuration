...
|untrusted_ip|Actual IP address of connecting client or peer which has not been authenticated yet. Sometimes used to nmap the connecting host in a --tls-verify script to ensure it is firewalled properly. Set prior to execution of --tls-verify and --auth-user-pass-verify scripts.|
...
# 在表status中更新用户登陆状态为0（离线），并更新数据信息（时间，流量等）
sqlcmd -S $HOST -U $USER -P $PASS -d $DB -Q "update status set user_online=0,last_logout=getdate(),last_total=$bytes_received+$bytes_sent,total_received=total_received+$bytes_received,total_sent=total_sent+$bytes_sent,grand_total=grand_total+$bytes_received+$bytes_sent where user_id='$common_name' and last_server='$SERV_N' and user_id not in (select user_id from log where user_id='$common_name' and log_conn_server!='$SERV_N' and log_note='client-connect')"
