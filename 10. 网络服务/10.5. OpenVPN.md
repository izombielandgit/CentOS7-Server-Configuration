...
firewall-cmd --permanent --direct --add-rule ipv4 filter INPUT 0 -i tun+ -j ACCEPT
...
-A POSTROUTING -o eth0 -d 192.168.10.2 -i tun0 -s 10.8.0.6 -j MASQUERADE
# 下面一步一步扩大适用范围, 根据自己需要来选择
-A POSTROUTING -o eth0 -d 192.168.10.2 -i tun0 -j MASQUERADE
-A POSTROUTING -o eth0 -i tun0 -j MASQUERADE
-A POSTROUTING -i tun0 -j MASQUERADE
-A POSTROUTING -o eth0 -j MASQUERADE
-A POSTROUTING -j MASQUERADE
COMMIT

*filter
# 表示INPUT, FORWARD, OUTPUT表默认策略是ACCEPT
:INPUT ACCEPT [0:0]
:FORWARD ACCEPT [0:0]
:OUTPUT ACCEPT [0:0]
# 服务器可以ping出
...
-A INPUT -p udp --dport 1194 -j ACCEPT
# 系统默认规则
# 意思是在INPUT表拒绝所有其他不符合上述任何一条规则的数据包, 并且发送一条host prohibited的消息给被拒绝的主机
-A INPUT -j REJECT --reject-with icmp-host-prohibited
-A INPUT -j DROP
# 添加以下规则
...
-A FORWARD -i tun0 -o eth0 -s 10.8.0.0/24 -m iprange --dst-range 192.168.10.2-192.168.0.5 -p tcp --dport 3389 -j DROP
-A FORWARD -i tun0 -o eth0 -s 10.8.0.0/24 -m iprange --dst-range 192.168.10.2-192.168.0.5 -j ACCEPT
# 意思是在FORWARD表拒绝所有其他不符合上述任何一条规则的数据包, 并且发送一条host prohibited的消息给被拒绝的主机
-A FORWARD -j REJECT --reject-with icmp-host-prohibited
COMMIT
...
让编译安装的pam-mysql支持MD5密码（参考这里<https://bugs.launchpad.net/ubuntu/+source/pam-mysql/+bug/1006005>，Lele Long用户的回复）：
...
>
> set session sql\_mode='ONLY\_FULL\_GROUP\_BY,STRICT\_TRANS\_TABLES,ERROR\_FOR\_DIVISION\_BY\_ZERO,NO\_AUTO\_CREATE\_USER,NO\_ENGINE\_SUBSTITUTION';
>
> CREATE TABLE `openvpn`.`log` \(  
>       `log_id` bigint\(20\) UNSIGNED NOT NULL AUTO\_INCREMENT,  
...
>       `log_start_time` timestamp NOT NULL DEFAULT CURRENT\_TIMESTAMP,  
...
编辑`/etc/openvpn/script/up.sh`文件，创建客户端连接时运行脚本：
