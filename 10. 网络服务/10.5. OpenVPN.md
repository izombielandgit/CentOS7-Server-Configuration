...
查了下资料，说是pam-mysql内存泄漏导致（参考这里<http://sourceforge.net/p/pam-mysql/bugs/27/#c2aa>，Dmitry Mikhailov的回复和Johannes Pahl的回复，指向<https://dev.mysql.com/doc/refman/5.7/en/mysql-library-init.html>）。
...
修改后能正常认证，~~是否还会出现该问题待测试~~使用了一段时间未出现问题（以前CentOS 6上安装时没有这样处理也没有发生过这种问题，以后CentOS有新版本不知道会不会更多坑）。
