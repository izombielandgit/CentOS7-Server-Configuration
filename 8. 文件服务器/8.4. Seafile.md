...
* 统一登录：默认情况下，企业微信用户扫码即可登录 Seafile 系统。支持两种模式
  * 导入后才可以扫码登录: 管理员在后台从企业微信导入用户后，用户才可扫码登录。
  * 扫码后自动导入到户: 用户扫码后，如果企业微信中有对应的用户，那么导入相关的信息，并让用户登录
...
登录企业微信管理后台 -> 应用管理 -> 应用 -> 自建 -> 创建应用。创建好后，进入应用后在"开发者接口"下的"企业微信授权登录"点"设置"，在"Web网页"下点击"设置授权回调域"，域名为Seafile服务器域名(如果使用的非标准端口也需要把端口带上)

配置该应用的“可见范围”，这决定着哪些企业微信的用户可以通过管理员直接导入到Seafile中

最后，收集上述应用的AgentId、Secret以及企业ID(我的企业 -> 企业信息 -> 企业ID)。

**配置Seahub**

**基础配置**

如果要集成企业微信认证登录，必须在`seahub_settings.py`中添加以下基础配置信息：

```
# Enable Work Weixin
ENABLE_WORK_WEIXIN = True                 # 开启企业微信认证登录功能
WORK_WEIXIN_CORP_ID = 'your-corpid'       # 企业ID
WORK_WEIXIN_AGENT_SECRET = 'your-secret'  # 应用secret
WORK_WEIXIN_AGENT_ID = 'your-agentid'     # 应用agentid
```

完成上述配置后，即可实现企业微信中的用户扫码注册并自动激活Seafile中的账号。这意味着无需经过Seafile系统管理员的允许，该用户就可以拥有注册并登录Seafile的权限。

如果您需要Seafile管理员严格掌控系统的注册权限，需要更多的高级配置。

**高级配置**

以下配置为可选项，您可按需在`seahub_settings.py`中配置修改：
...
重启成功后，可以通过点击登录页面上的“单点登录”来使用企业微信扫码登录。

**导入企业微信用户**

上述过程执行成功后，或许还需要事先导入并激活企业微信中的用户（如果设置了`WORK_WEIXIN_OAUTH_CREATE_UNKNOWN_USER = False`）。

使用Seafile管理员账号，登录到“系统管理 -> 企业微信集成”页面。勾选需要导入的用户后，点击“导入用户”即可。(前面创建企业微信应用时，"可见范围"如果是选择的个人而非部门，在"企业微信集成"页面无法拉取到信息)

注意：导入的用户均为“未激活”状态，需要管理员手动激活该用户。

**关联企业微信账号**

如果在Seafile系统中已经创建了一个普通账号，可以选择绑定一个企业微信账号，实现企业微信认证登录。
...
集成企业微信认证登录的功能依赖于企业微信对外发布的API，如果企业微信API URL有变动，可通过在seahub_settings.py中添加以下配置修改：
...
注意：上述URL为配置示例，可查询企业微信API文档获取最新的API URL。
